...
const StagingProductDetail: React.FC<RouteComponentProps> = (props) => {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      await saveStagingProduct(product);

      setLoading(false);
      message.success("Register updated with success.");
      history.push("/staging-list");
    } catch (error) {
      console.error(error);
      setLoading(false);
    }
      ...
      >
        ...
        <Row gutter={8}>
          <Col>
            <Button
              type="default"
              onClick={() => history.push("/staging-list")}
            >
              Cancel
            </Button>
          </Col>
...
export default StagingProductDetail;
