...
const DirectLinkDetail: React.FC<DirectLinkDetailProps> = ({
  ...
  const [userInput, setUserInput] = useState<string | undefined>(directLink?.product?.name);
  ...
  const [selectedProductBrand, setSelectedProductBrand] = useState<ProductBrand>();
  ...
  const onFinish = async () => {
    try {
      ...
      if (item.creator) 
        item.creator = {id: selectedCreator!.id, userName: selectedCreator!.userName, firstName: selectedCreator!.firstName};

      if (item.masterBrand) 
        item.masterBrand = {id: selectedMasterBrand!.id, brandName: selectedMasterBrand!.brandName};

      if (item.productBrand) 
        item.productBrand = {id: selectedProductBrand!.id, brandName: selectedProductBrand!.brandName};
  ...
  return (
    <div>
      <PageHeader title={directLink ? 'Direct Link Update' : 'New Direct Link'} />
        <Form
          form={form}
          onFinish={onFinish}
          name="form"
