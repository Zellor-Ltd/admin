...
import CreatorsPageDetail from './CreatorsPageDetail';
...
const CreatorsPage: React.FC<RouteComponentProps> = ({ location }) => {
      ...
      render: (_, record, index: number) => (
        <>
          <Link
            ...
            onClick={() => editMasthead(index, record)}
          >
            <EditOutlined />
          </Link>
          <Popconfirm
  ...
  const editMasthead = (index: number, masthead?: Masthead) => {
        ...
        <div className="Mastheads">
          <PageHeader
            ...
            extra={[
              <Button
                key="1"
                onClick={() => editMasthead(filteredMastheads.length)}
              >
                New Item
              </Button>,
            ]}
